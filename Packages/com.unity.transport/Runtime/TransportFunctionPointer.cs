...
namespace Unity.Networking.Transport
{
    ...
    public struct TransportFunctionPointer<T> where T : Delegate
    {
        ...
        public TransportFunctionPointer(T executeDelegate)
        {
            ...
            {
                GCHandle.Alloc(executeDelegate); // Ensure delegate is never garbage-collected.
                Ptr = new FunctionPointer<T>(Marshal.GetFunctionPointerForDelegate(executeDelegate));
            }
        }
        ...
        {
            return new TransportFunctionPointer<T>(burstCompilableDelegate);
        }
